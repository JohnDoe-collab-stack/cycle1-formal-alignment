...
def runDecisionFlip
  ...
  match comparison.equal with
  | true =>
      { output := { var := decision.var, value := !decision.value }
        variableConstructorVisits := comparison.constructorVisits }
  | false =>
      { output := decision
        variableConstructorVisits := comparison.constructorVisits }
...
theorem runDecisionFlip_output
    ...
    (runDecisionFlip selected decision).output =
      StructuralBranchDecision.flipAt selected decision := by
  unfold runDecisionFlip StructuralBranchDecision.flipAt
  cases comparison : (runNatComparison decision.var selected).equal with
  | false =>
      have different : decision.var ≠ selected := by
        intro same
        cases same
        rw [runNatComparison_equal_implies_true] at comparison
        cases comparison
      change
        (match (runNatComparison decision.var selected).equal with
        | true =>
            { var := decision.var, value := !decision.value }
        | false => decision) =
          (if decision.var = selected then
            { var := decision.var, value := !decision.value }
          else
            decision)
      rw [comparison, if_neg different]
  | true =>
      have same : decision.var = selected :=
        runNatComparison_true_implies_equal _ _ comparison
      change
        (match (runNatComparison decision.var selected).equal with
        | true =>
            { var := decision.var, value := !decision.value }
...
theorem runDecisionHistoryComparison_pathVariablesFrom_decisionVisits
  ...
  | zero =>
      have markerFalse :
        ...
        simp [
          runDecisionComparison,
          runNatComparison_false_of_ne firstMarker secondMarker different
        ]
      simp only [pathVariablesFrom, falseDecisionHistory]
...
def retainDiscoveryTrace
    {rootFormula : Cnf}
    {state : GeneratedStructuralBranchContext rootFormula}
    (run : EndogenousDiscoveryRun state) : RetainedDiscoveryTrace :=
...
/-- The only trace admitted by the integrated benchmark is computed from input. -/
def operationalDiscoveryTrace (input : Nat) : RetainedDiscoveryTrace :=
  retainDiscoveryTrace
    (runEndogenousFlipDiscovery (distinctGrowingDiscoveryRoot input))
...
theorem operationalDiscoveryTrace_selected
  ...
  unfold operationalDiscoveryTrace retainDiscoveryTrace
  rw [found]
...
structure OperationalDiscoverySchedule (input : Nat) where
  ...
  selected : Var
  selectedFromDiscovery : discovery.selected? = some selected
...
def scheduleOperationalFromDiscovery
  ...
  let discovery := operationalDiscoveryTrace input
  match selectedExact : discovery.selected? with
  | none => none
  | some selected =>
      some
        { discovery := discovery
          selected := selected
          selectedFromDiscovery := selectedExact
...
theorem runIntegratedOperational_reconstructible
  ...
  · rename_i missing
    ...
    cases selected
  · rename_i selectedVariable selectedExact
    have canonical := operationalDiscoveryTrace_selected input
    have same : selectedVariable = growingDiscoverySplitVar input :=
      Option.some.inj (Eq.trans selectedExact.symm canonical)
    cases same
    simpa [
      operationalTarget,
      operationalPositiveRun,
...
def emptyRetainedDiscoveryTrace : RetainedDiscoveryTrace :=
  { candidates := []
    extractionStats := { clauseVisits := 0, literalVisits := 0, candidatesEmitted := 0 }
    attempts := 0
    selected? := none }

def selectedRetainedDiscoveryTrace : RetainedDiscoveryTrace :=
  { candidates := [0]
    extractionStats := { clauseVisits := 1, literalVisits := 1, candidatesEmitted := 1 }
    attempts := 1
    selected? := some 0 }
...
#print ConstitutiveSearch.SAT.runLiteralFlip_output
