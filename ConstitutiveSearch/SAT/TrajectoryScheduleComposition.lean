...
/--
Two consecutive local sibling reductions of a flip-symmetric trajectory do not
share an endpoint.

The first target is the retained true child at depth d+1.
The second source is a false child generated from that retained child at depth
d+2.
-/
theorem consecutiveLocalWitnesses_endpoint_gap
    {rootFormula : Cnf}
    {parent finish :
